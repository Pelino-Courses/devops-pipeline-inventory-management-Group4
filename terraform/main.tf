# Azure Provider Configuration (assuming this is in versions.tf or provider.tf)
# You must have the required provider blocks and variables defined elsewhere.
...
resource "azurerm_network_security_group" "main" {
  ...
  # Allow SSH (Port 22) - WARNING: Open to the world (0.0.0.0/0)
  security_rule {
    name                       = "Allow-SSH"
    priority                   = 100
    direction                  = "Inbound"
    ...
    source_address_prefix      = "*" # Open to all, see compatibility check below
...
resource "azurerm_linux_virtual_machine" "main" {
  ...
  # Mandatory for Managed Identity usage
  identity {
    type = "SystemAssigned"
  }
